...
def __tr(txt, disambiguation=None, n=-1):
    return QApplication.translate('InfoBar', txt, disambiguation, n)
...
def show_result_toast(results, _type='scan'):
    ...
    if _type == 'scan':
        ...
        verb = __tr('extracting')
    fail_message = __tr(f'Finished {verb} ba2. Could not open {num_fail} files')
    # fail_message = __tr('Finished {0} ba2. Could not open {1} files').format(verb, num_fail)
    if auto_ignore:
        fail_message += __tr(' and they will be ignored in the future.')
    else:
        fail_message += __tr('.')
    if _type == 'scan':
    ...
    if _type == 'scan':
        fail_title = __tr('Failed to load some files')
    else:
        fail_title = __tr('Failed to extract some files')
    if num_fail > 0:
        ...
        more_info_button = PushButton(__tr('Details'), warning_info)
        more_info_button.clicked.connect(lambda: show_failed_files(results[0]))
        warning_info.addWidget(more_info_button)
        warning_info.show()
    else:
        if _type == 'scan':
            success_title = __tr('Ready')
            success_message = __tr(f'Finished scanning ba2. Processed {num_success} files.')
            num_ignored = results[3]
            if num_ignored > 0:
                success_message += __tr(f' Skipped {num_ignored} ignored files.')
        else:
            success_title = __tr('All set')
            success_message = __tr(f'Finished extracting ba2. Extracted {num_success} files.')
        InfoBar.success(
            title=success_title,
